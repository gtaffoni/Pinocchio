...
int count_peaks(int *ngood)
{
  ...
  *ngood = 0;      /* number of groups out of the safety boundary */
  ...
  for (iz = 0; iz < subbox.Nstored; iz++)
  {
    ...
    /* avoid borders */
    if (!subbox.pbc[_x_] && (i == 0 || i == subbox.Lgwbl[_x_] - 1))
      continue;
    if (!subbox.pbc[_y_] && (j == 0 || j == subbox.Lgwbl[_y_] - 1))
      continue;
    if (!subbox.pbc[_z_] && (k == 0 || k == subbox.Lgwbl[_z_] - 1))
      continue;
...
double compute_Nhalos_in_PLC(double z1, double z2)
{
  ...
  do
  {
    ...
    number += result * solidangle * (pow(ComovingDistance(upper), 3.) - pow(ComovingDistance(lower), 3.)) / 3.;
    lower += delta_z;
  } while (upper < z2);
