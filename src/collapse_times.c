...
#if !defined(GPU_OMP_FULL) && !defined(GPU_OMP)
...
// #define CT_NBINS_XY (128) 
// #define CT_NBINS_D (256) 
// #endif
...
int initialize_collapse_times(int ismooth, int onlycompute){	
	...
	if (!ismooth){
		...
		#ifdef CUSTOM_INTERPOLATION
    	/* Allocate memory for cubic spline array */
    	CT_Spline = (CubicSpline **)malloc(CT_NBINS_XY * CT_NBINS_XY * sizeof(CubicSpline *));

    	/* Initialize each spline and allocate memory for its internal arrays */
	...
	/* Fill each spline  and offload their internal values */
    for (int i = 0; i < CT_NBINS_XY; ++i) {
        for (int j = 0; j < CT_NBINS_XY; ++j) {
            int index = i * CT_NBINS_XY + j; // Flattened index calculation

            /* Initialize the cubic spline with the allocated structure */
            custom_cubic_spline_init(CT_Spline[index], delta_vector,
                &CT_table[i * CT_NBINS_D + j * CT_NBINS_D * CT_NBINS_XY], CT_NBINS_D);
        }
    }
...
FORCE_INLINE double interpolate_collapse_time(int ismooth, double l1, double l2, double l3){
	...
	return  ((1. - dx) * (1. - dy) * custom_cubic_spline_eval(CT_Spline[ix * CT_NBINS_XY + iy], d) +
		    (dx) * (1. - dy) * custom_cubic_spline_eval(CT_Spline[(ix + 1) * CT_NBINS_XY + iy], d)  +
		    (1. - dx) * (dy) * custom_cubic_spline_eval(CT_Spline[ix * CT_NBINS_XY + (iy + 1)], d)  +
	    	(dx) * (dy) * custom_cubic_spline_eval(CT_Spline[(ix + 1) * CT_NBINS_XY + (iy + 1)], d)) ;
...
#endif // defined(TABULATED_CT) && !defined(CUSTOM_INTERPOLATION) && !defined(GPU_OMP_FULL)
